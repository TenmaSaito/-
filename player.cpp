...
void InitPlayer(void)
{
	...
	g_player.posHead = D3DXVECTOR3(g_player.pos.x, g_player.pos.y - PLAYER_HEIGHT, 0.0f);		// 位置を初期化
	g_player.posHeadOld = D3DXVECTOR3(g_player.pos.x, g_player.pos.y - PLAYER_HEIGHT, 0.0f);	// 位置を初期化
...
void UpdatePlayer(void)
{
	...
	{ // Aキーなら
		...
		if (g_player.bJump) g_player.move.x -= PLAYER_SPD * 0.25f;	// 加速度追加
	...
	{ // 位置を調整し、ジャンプ可能に
		...
		g_player.bGravityInverseTime = true;	// 反転のロックを解除する
	}
	else
	{
		g_player.bGravityInverseTime = false;	// 反転のロックを解除する
