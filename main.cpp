...
#define WINDOW_NAME		"暗黒破壊神ステージエディタ―マン"					// キャプションに表示される名前(提出する前に変えること！！！)
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_RBUTTONDOWN:
		...
		//ポップアップメニューを表示
		TrackPopupMenu(GetSubMenu(GetMenu(hWnd), 0), TPM_LEFTALIGN, ptMouse.x, ptMouse.y, 0, hWnd, NULL);
		break;
	...
	case WM_COMMAND: //コントロールの操作
		...
		case ID_NAMEDSAVE:
			// ofnに値をセット.
			// strcpy(szFileTitle, "UI");
			GetWindowText(hWnd, szFileTitle, sizeof(char) * MAX_TITLE);
		...
		case ID_OPENFILE:
			...
			oFileName.nMaxFile = MAX_PATH;												// 上記の文字列のデータの最大数
			oFileName.Flags = OFN_FILEMUSTEXIST | OFN_HIDEREADONLY;						// フラグ(開く際の動作)
...
void DrawDebug(void)
{
	...
	sprintf(&aStr[1][0], "U / O : 高さ変更 (%f) \nN / M : 幅変更 (%f)\njump %d\npos %f %f\nposOld %f %f\nBpos %f %f\nCollison %d", GetBlock()->fHeight, GetBlock()->fWidth, GetPlayer()->bJump, GetPlayer()->pos.x, GetPlayer()->pos.y, GetPlayer()->posOld.x, GetPlayer()->posOld.y, GetBlock()->pos.x, GetBlock()->pos.y, GetCollison());
